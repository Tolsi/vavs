// DON'T EDIT THIS FILE.
// This file is auto generated by sbt-lock 0.2.3.
// https://github.com/tkawachi/sbt-lock/
dependencyOverrides in ThisBuild ++= Set(
  "ch.qos.logback" % "logback-classic" % "1.1.8",
  "ch.qos.logback" % "logback-core" % "1.1.8",
  "com.chuusai" % "shapeless_2.12" % "2.3.2",
  "com.datadoghq" % "java-dogstatsd-client" % "2.1.1",
  "com.fasterxml" % "classmate" % "1.1.0",
  "com.fasterxml.jackson.core" % "jackson-annotations" % "2.7.6",
  "com.fasterxml.jackson.core" % "jackson-core" % "2.7.6",
  "com.fasterxml.jackson.core" % "jackson-databind" % "2.7.6",
  "com.fasterxml.jackson.datatype" % "jackson-datatype-guava" % "2.6.3",
  "com.fasterxml.jackson.datatype" % "jackson-datatype-jdk7" % "2.6.3",
  "com.fasterxml.jackson.datatype" % "jackson-datatype-jdk8" % "2.7.6",
  "com.fasterxml.jackson.datatype" % "jackson-datatype-joda" % "2.6.3",
  "com.fasterxml.jackson.datatype" % "jackson-datatype-jsr310" % "2.7.6",
  "com.fasterxml.jackson.module" % "jackson-module-afterburner" % "2.6.3",
  "com.github.mpilquist" % "simulacrum_2.12" % "0.10.0",
  "com.github.nscala-time" % "nscala-time_2.12" % "2.14.0",
  "com.google.code.findbugs" % "jsr305" % "3.0.1",
  "com.google.guava" % "guava" % "20.0",
  "com.h2database" % "h2-mvstore" % "1.4.193",
  "com.typesafe" % "config" % "1.3.1",
  "com.typesafe" % "ssl-config-core_2.12" % "0.2.1",
  "com.typesafe.akka" % "akka-actor_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-cluster-tools_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-cluster_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-contrib_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-http-core_2.12" % "10.0.0",
  "com.typesafe.akka" % "akka-http_2.12" % "10.0.0",
  "com.typesafe.akka" % "akka-parsing_2.12" % "10.0.0",
  "com.typesafe.akka" % "akka-persistence_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-protobuf_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-remote_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-slf4j_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-stream_2.12" % "2.4.14",
  "com.typesafe.akka" % "akka-testkit_2.12" % "2.4.14",
  "com.typesafe.play" % "play-datacommons_2.11" % "2.5.9",
  "com.typesafe.play" % "play-functional_2.11" % "2.5.9",
  "com.typesafe.play" % "play-iteratees_2.11" % "2.5.9",
  "com.typesafe.play" % "play-json_2.11" % "2.5.9",
  "commons-codec" % "commons-codec" % "1.6",
  "commons-logging" % "commons-logging" % "1.1.3",
  "commons-net" % "commons-net" % "3.5",
  "io.aeron" % "aeron-client" % "1.0.4",
  "io.aeron" % "aeron-driver" % "1.0.4",
  "io.dropwizard" % "dropwizard-jackson" % "0.9.1",
  "io.dropwizard" % "dropwizard-lifecycle" % "0.9.1",
  "io.dropwizard" % "dropwizard-metrics" % "0.9.1",
  "io.dropwizard" % "dropwizard-util" % "0.9.1",
  "io.dropwizard" % "dropwizard-validation" % "0.9.1",
  "io.dropwizard.metrics" % "metrics-core" % "3.1.2",
  "io.dropwizard.metrics" % "metrics-jvm" % "3.1.2",
  "io.netty" % "netty" % "3.10.6.Final",
  "io.reactivex" % "rxjava" % "1.2.2",
  "io.reactivex" % "rxscala_2.12" % "0.26.4",
  "javax.servlet" % "javax.servlet-api" % "3.1.0",
  "javax.validation" % "validation-api" % "1.1.0.Final",
  "jline" % "jline" % "2.14.1",
  "joda-time" % "joda-time" % "2.9.4",
  "org.agrona" % "Agrona" % "0.9.0",
  "org.apache.httpcomponents" % "fluent-hc" % "4.3.6",
  "org.apache.httpcomponents" % "httpclient" % "4.3.6",
  "org.apache.httpcomponents" % "httpcore" % "4.3.3",
  "org.consensusresearch" % "scrypto_2.12" % "1.2.0-RC3",
  "org.coursera" % "dropwizard-metrics-datadog" % "1.1.6",
  "org.coursera" % "metrics-datadog" % "1.1.6",
  "org.eclipse.jetty" % "jetty-http" % "9.2.13.v20150730",
  "org.eclipse.jetty" % "jetty-io" % "9.2.13.v20150730",
  "org.eclipse.jetty" % "jetty-server" % "9.2.13.v20150730",
  "org.eclipse.jetty" % "jetty-util" % "9.2.13.v20150730",
  "org.glassfish" % "javax.el" % "3.0.0",
  "org.hibernate" % "hibernate-validator" % "5.2.2.Final",
  "org.jboss.logging" % "jboss-logging" % "3.2.1.Final",
  "org.joda" % "joda-convert" % "1.8.1",
  "org.reactivestreams" % "reactive-streams" % "1.0.0",
  "org.scala-lang.modules" % "scala-async_2.12" % "0.9.6",
  "org.scala-lang.modules" % "scala-java8-compat_2.12" % "0.8.0",
  "org.scala-lang.modules" % "scala-parser-combinators_2.12" % "1.0.4",
  "org.scala-lang.modules" % "scala-xml_2.12" % "1.0.6",
  "org.scala-sbt" % "test-interface" % "1.0",
  "org.scala-stm" % "scala-stm_2.11" % "0.7",
  "org.scalacheck" % "scalacheck_2.12" % "1.13.4",
  "org.scalactic" % "scalactic_2.12" % "3.0.1",
  "org.scalatest" % "scalatest_2.12" % "3.0.1",
  "org.slf4j" % "slf4j-api" % "1.7.21",
  "org.spire-math" % "kind-projector_2.12" % "0.9.3",
  "org.typelevel" % "catalysts-macros_2.12" % "0.0.5",
  "org.typelevel" % "catalysts-platform_2.12" % "0.0.5",
  "org.typelevel" % "cats-core_2.12" % "0.8.1",
  "org.typelevel" % "cats-free_2.12" % "0.8.1",
  "org.typelevel" % "cats-jvm_2.12" % "0.8.1",
  "org.typelevel" % "cats-kernel-laws_2.12" % "0.8.1",
  "org.typelevel" % "cats-kernel_2.12" % "0.8.1",
  "org.typelevel" % "cats-laws_2.12" % "0.8.1",
  "org.typelevel" % "cats-macros_2.12" % "0.8.1",
  "org.typelevel" % "cats_2.12" % "0.8.1",
  "org.typelevel" % "discipline_2.12" % "0.7.2",
  "org.typelevel" % "machinist_2.12" % "0.6.1",
  "org.typelevel" % "macro-compat_2.12" % "1.1.1",
  "org.uncommons.maths" % "uncommons-maths" % "1.2.2a",
  "org.whispersystems" % "curve25519-java" % "0.3.0"
)
// LIBRARY_DEPENDENCIES_HASH 027b68202ffaa0d9a7c15d1573e5dd97686589a7
